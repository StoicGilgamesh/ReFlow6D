...
def batch_data(cfg, data, renderer=None, device="cuda", phase="train"):
    ...
    batch["flow_img"] = torch.stack([d["flow_img"] for d in data], dim=0).to(device, non_blocking=True)
    batch["rho_img"] = torch.stack([d["rho_img"] for d in data], dim=0).to(device, non_blocking=True)
    # INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
...
def batch_data_train_online(cfg, data, renderer, device="cuda"):
    ...
    # INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
    batch["flow_img"] = torch.stack([d["flow_img"] for d in data], dim=0).to(device, non_blocking=True)
    np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_input/batch_flow_img.npy", batch["flow_img"].cpu().detach().numpy())
    batch["rho_img"] = torch.stack([d["rho_img"] for d in data], dim=0).to(device, non_blocking=True)
    batch["image_file_name"] = np.stack([d["image_file_name"] for d in data])
    # INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
    if cfg.INPUT.WITH_DEPTH:
        batch["roi_depth"] = torch.stack([d["roi_depth"] for d in data], dim=0).to(device, non_blocking=True)
    batch["roi_cls"] = torch.as_tensor([d["roi_cls"] for d in data], dtype=torch.long).to(device, non_blocking=True)
    ...
    # print("*******************************************************: ", [d["file_path"] for d in data])
    # print("\n$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$: ", [d["anno_i"] for d in data])
    # batch["roi_file_path"] = torch.as_tensor([d["file_path"] for d in data]) #################################################### roi_file_path roi_file_pathroi_file_pathroi_file_path
    ...
    # ).to(torch.float32)
    batch["roi_mask_obj"] = torch.stack([d["roi_mask_obj"] for d in data], dim=0).to( device=device, dtype=torch.float32, non_blocking=True)
    
    batch["roi_xyz"] = torch.stack([d["roi_xyz"] for d in data], dim=0).to( device=device, dtype=torch.float32, non_blocking=True)
    if "roi_xyz_bin" in data[0]:
        batch["roi_xyz_bin"] = torch.stack([d["roi_xyz_bin"] for d in data], dim=0).to( device=device, dtype=torch.float32, non_blocking=True)
    batch["roi_region"] = torch.stack([d["roi_region"] for d in data], dim=0).to( device=device, dtype=torch.long, non_blocking=True)
    #np.save("/RoadmapWithStefan/gdrnpp_bop2022/batch_roi_mask_trunc.npy", batch["roi_mask_trunc"].cpu().detach().numpy())
    np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/roi_xyz.npy", batch["roi_xyz"].cpu().detach().numpy())
    #np.save("/RoadmapWithStefan/gdrnpp_bop2022/batch_roi_mask_visib.npy", batch["roi_mask_visib"].cpu().detach().numpy())
    #np.save("/RoadmapWithStefan/gdrnpp_bop2022/batch_roi_mask_obj.npy", batch["roi_mask_obj"].cpu().detach().numpy())

############################## THIS HAS TO BE CHANGED TO ORIGINAL ########################################################################################################
    batch["roi_mask_trunc"] = batch["roi_mask_trunc"] * batch["roi_mask_obj"]
    batch["roi_mask_visib"] = batch["roi_mask_visib"] * batch["roi_mask_obj"]
    # batch["roi_mask_trunc"] = batch["roi_mask_trunc"]
    # batch["roi_mask_visib"] = batch["roi_mask_visib"]

############################## THIS HAS TO BE CHANGED TO ORIGINAL ########################################################################################################
    #np.save("/RoadmapWithStefan/gdrnpp_bop2022/batch_roi_mask_trunc_after.npy", batch["roi_mask_trunc"].cpu().detach().numpy())
    #np.save("/RoadmapWithStefan/gdrnpp_bop2022/batch_roi_mask_visib_after.npy", batch["roi_mask_visib"].cpu().detach().numpy())
    #np.save("/RoadmapWithStefan/gdrnpp_bop2022/batch_roi_mask_obj_after.npy", batch["roi_mask_obj"].cpu().detach().numpy())
   ...
   # np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug/roi_region_batch.npy", batch["roi_region"].cpu().detach().numpy())
   # np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug/roi_fps_points_batch.npy", batch["roi_fps_points"].cpu().detach().numpy())
   # np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug/roi_mask_obj_batch.npy", batch["roi_mask_obj"].cpu().detach().numpy())
   # np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug/roi_mask_trunc_batch.npy", batch["roi_mask_trunc"].cpu().detach().numpy())
   # np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug/roi_mask_visib_batch.npy", batch["roi_mask_visib"].cpu().detach().numpy())
   # np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug/roi_mask_trunc_batch_mult.npy", (batch["roi_mask_trunc"] * batch["roi_mask_obj"]).cpu().detach().numpy())
   # np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug/roi_mask_visib_batch_mult.npy", (batch["roi_mask_visib"] * batch["roi_mask_obj"]).cpu().detach().numpy())
   ...
   ############################################### FOR MATTING LOSS ##############################################################
    # batch["matting_coco_img"] = torch.stack([d["matting_coco_img"] for d in data], dim=0).to(device, non_blocking=True)
    batch["matting_coco_img"] = np.stack([d["matting_coco_img"] for d in data], axis=0)
   ############################################### FOR MATTING LOSS ##############################################################
   # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ENGINE_UTILS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
   # import sys
   # sys.exit()
...
def get_renderer(cfg, data_ref, obj_names, gpu_id=None):
   ...
   # print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
   # print("obj_idsobj_idsobj_idsobj_idsobj_idsobj_idsobj_idsobj_idsobj_idsobj_idsobj_idsobj_idsobj_idsobj_idsobj_idsobj_ids: ",obj_ids)
   # print("model_pathsmodel_pathsmodel_pathsmodel_pathsmodel_pathsmodel_pathsmodel_pathsmodel_pathsmodel_pathsmodel_pathsmodel_paths: ",model_paths)
   # print("texture_pathstexture_pathstexture_pathstexture_pathstexture_pathstexture_pathstexture_pathstexture_pathstexture_paths: ",texture_paths)
   # print("data_ref.vertex_scaledata_ref.vertex_scaledata_ref.vertex_scaledata_ref.vertex_scaledata_ref.vertex_scaledata_ref.vertex_scale: ", data_ref.vertex_scale)
   # print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
    ren = EGLRenderer(
        model_paths,
