    ...
    def do_train(self, cfg, args, model, optimizer, renderer=None, resume=False):
        ...
        with EventStorage(start_iter) as storage:
            ...
            for iteration in range(start_iter, max_iter):
                ...
                # EDIT MODEL INPUT FOR TRANSPARENT OBJECTS
                out_dict, loss_dict = model(
                    inp,
                    # roi_file_path=batch["roi_file_path"],
                    gt_flow_img=batch["flow_img"],
                    gt_rho_img=batch["rho_img"],
                    gt_mask_trunc=batch["roi_mask_trunc"],
                    gt_mask_visib=batch["roi_mask_visib"],
                    gt_mask_full=batch.get("roi_mask_full", None),
                    gt_mask_obj=batch["roi_mask_obj"],
                    ...
                    roi_extents=batch.get("roi_extent", None),
                    do_loss=True,
                    background=batch["matting_coco_img"] ############################## MATTING LOSSS ##################################
                )
                ...
                # out_rots = out_dict["rot"].cpu().detach().numpy()
                # out_transes = out_dict["trans"].cpu().detach().numpy()
                
                # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/out_rots.npy", out_rots)
                # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/out_transes.npy", out_transes)
                # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/image_file_name.npy", batch["image_file_name"])
                # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ENGINE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                # import sys
                # sys.exit()
                ...
                if iteration - start_iter > 5 and (
                    ...
                    if cfg.TRAIN.VIS_IMG:
                        with torch.no_grad():
                            ...
                            out_flow = out_dict["flow"].detach()
                            # out_flow = get_out_mask(cfg, out_flow)
                            out_flow_vis = out_flow[vis_i, 0].cpu().numpy()
                            tbx_writer.add_image("out_flow", out_flow_vis, iteration)

                            gt_flow_img_vis = batch["flow_img"][vis_i].detach().cpu().numpy()
                            tbx_writer.add_image("gt_flow_img", gt_flow_img_vis, iteration)
                            
                            
                            out_rho = out_dict["rho"].detach()
                            # out_rho = get_out_mask(cfg, out_rho)
                            out_rho_vis = out_rho[vis_i, 0].cpu().numpy()
                            tbx_writer.add_image("out_rho", out_rho_vis, iteration)

                            gt_rho_img_vis = batch["rho_img"][vis_i].detach().cpu().numpy()
                            tbx_writer.add_image("gt_rho_img", gt_rho_img_vis, iteration)
