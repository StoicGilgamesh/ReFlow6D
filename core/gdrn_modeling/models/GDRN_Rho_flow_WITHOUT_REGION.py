...
class GDRN_Rho_flow(nn.Module):
    def __init__(self, cfg, backbone, geo_head_net, neck=None, pnp_net=None):
        ...
        #self.flow_out_dim, self.rho_out_dim, self.mask_out_dim = get_xyz_mask_flow_rho_region_out_dim(cfg)
        self.flow_out_dim, self.rho_out_dim, self.mask_out_dim = get_xyz_mask_flow_rho_out_dim(cfg)
    ...
    def forward(
        ...
        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/x.npy",x.cpu().detach().numpy())
        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/gt_flow_img.npy",gt_flow_img.cpu().detach().numpy())
        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/gt_rho_img.npy",gt_rho_img.cpu().detach().numpy())
        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/gt_mask_trunc.npy",gt_mask_trunc.cpu().detach().numpy())
        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/gt_mask_obj.npy",gt_mask_obj.cpu().detach().numpy())
        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/gt_mask_visib.npy",gt_mask_visib.cpu().detach().numpy())
        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/gt_mask_full.npy",gt_mask_full.cpu().detach().numpy())
        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/gt_region.npy",gt_region.cpu().detach().numpy())
#        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/gt_ego_rot.npy",gt_ego_rot.cpu().detach().numpy())
#        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/gt_trans.npy",gt_trans.cpu().detach().numpy())
#        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/roi_coord_2d.npy",roi_coord_2d.cpu().detach().numpy())
#        np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_exp/roi_coord_2d_rel.npy",roi_coord_2d_rel)
#       print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! GDRN_RHO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
#        import sys
#        sys.exit()
        ...
        mask, flow, rho = self.geo_head_net(conv_feat)
        ...
        # region_softmax = F.softmax(gt_region[:, 1:, :, :], dim=1)
        region_softmax = gt_region[:, 1:, :, :]
        ...
        # pred_rot_, pred_t_ = self.pnp_net(
        #     flow=flow, rho=rho, extents=roi_extents, mask_attention=mask_atten
        # )
        ...
        if not do_loss:  # test
            ...
            if cfg.TEST.USE_PNP or cfg.TEST.SAVE_RESULTS_ONLY:
                ...
                out_dict.update({"mask": mask, "flow": flow, "rho": rho})
        else:
            out_dict = {}
            assert (
    ...
    def gdrn_loss(
        ...
        if not g_head_cfg.FREEZE:
            ...
            np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_output/out_flow.npy",(out_flow.cpu()).detach().numpy())
            np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_output/gt_mask_flow.npy",((gt_mask_flow[:, None]).cpu()).detach().numpy())
            ...
            np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_output/out_flow_x_mask_img.npy",(( out_flow * gt_mask_flow[:, None]).cpu()).detach().numpy())
            np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_output/gt_flow_x_mask_img.npy",((gt_flow_img*gt_mask_flow[:, None]).cpu()).detach().numpy())
            #print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! GDRN_RHO LOSS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            #import sys
            #sys.exit()    
        # rho loss ----------------------------------
        if not g_head_cfg.FREEZE:
            ...
            np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_output/out_rho.npy",(out_rho.cpu()).detach().numpy())
            np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_output/gt_rho_img.npy",(gt_rho_img.cpu()).detach().numpy())
            np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_output/gt_mask_rho.npy",(gt_mask_rho.cpu()).detach().numpy())
            np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_output/out_rho_x_mask_img.npy",(( out_rho * gt_mask_rho[:, None]).cpu()).detach().numpy())
        ...
        if not g_head_cfg.FREEZE:
            ...
            np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_output/out_mask.npy",( (out_mask[:, 0, :, :]).cpu()).detach().numpy())
            np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_output/gt_mask.npy",(gt_mask.cpu()).detach().numpy())
