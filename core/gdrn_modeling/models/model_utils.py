...
def get_xyz_mask_flow_rho_region_out_dim(cfg):
    net_cfg = cfg.MODEL.POSE_NET
    g_head_cfg = net_cfg.GEO_HEAD
    loss_cfg = net_cfg.LOSS_CFG
    ...
    if flow_loss_type in ["MSE", "L1", "L2", "SmoothL1"]:
      flow_out_dim = 3
    elif flow_loss_type in ["CE_coor", "CE"]:
        flow_out_dim = 3 
        # flow_out_dim = 3 * (g_head_cfg.XYZ_BIN + 1)
    else:
        raise NotImplementedError(f"unknown flow loss type: {flow_loss_type}")
        
    rho_loss_type = loss_cfg.RHO_LOSS_TYPE
    if rho_loss_type in ["MSE", "L1", "L2", "SmoothL1"]:
        rho_out_dim = 3
    elif rho_loss_type in ["CE_coor", "CE"]:
        rho_out_dim = 3 
        # rho_out_dim = 3 * (g_head_cfg.XYZ_BIN + 1)
    else:
    ...
    ########################################################## ABALATION STUDIES ###################################################################
    return flow_out_dim, rho_out_dim, mask_out_dim, region_out_dim
    # return mask_out_dim, region_out_dim
    ########################################################## ABALATION STUDIES ###################################################################

def get_xyz_mask_flow_rho_out_dim(cfg):
    net_cfg = cfg.MODEL.POSE_NET
    g_head_cfg = net_cfg.GEO_HEAD
    loss_cfg = net_cfg.LOSS_CFG
    ...
    return flow_out_dim, rho_out_dim, mask_out_dim
...
def get_geo_head(cfg):
    ...
    elif "FlowRho" in geo_head_type:
          flow_out_dim, rho_out_dim, mask_out_dim, region_out_dim = get_xyz_mask_flow_rho_region_out_dim(cfg)
    ########################################################## ABALATION STUDIES ###################################################################
         # mask_out_dim, region_out_dim = get_xyz_mask_flow_rho_region_out_dim(cfg)
    ########################################################## ABALATION STUDIES ###################################################################
         # region_num_classes = net_cfg.NUM_CLASSES if geo_head_cfg.REGION_CLASS_AWARE else 1
    ########################################################## ABALATION STUDIES ###################################################################
          flow_num_classes = net_cfg.NUM_CLASSES if geo_head_cfg.FLOW_CLASS_AWARE else 1
    ########################################################## ABALATION STUDIES ###################################################################
          region_num_classes = net_cfg.NUM_CLASSES if geo_head_cfg.REGION_CLASS_AWARE else 1
    ########################################################## ABALATION STUDIES ###################################################################
          rho_num_classes = net_cfg.NUM_CLASSES if geo_head_cfg.RHO_CLASS_AWARE else 1
    ########################################################## ABALATION STUDIES ###################################################################
          geo_head_init_cfg.update(
    ########################################################## ABALATION STUDIES ###################################################################
              flow_num_classes=flow_num_classes,
              rho_num_classes=rho_num_classes,
    ########################################################## ABALATION STUDIES ###################################################################
             mask_num_classes=mask_num_classes,
             region_num_classes=region_num_classes,
             mask_out_dim=mask_out_dim,
             region_out_dim=region_out_dim,
    ########################################################## ABALATION STUDIES ###################################################################
              flow_out_dim = flow_out_dim,
              rho_out_dim = rho_out_dim,
    ########################################################## ABALATION STUDIES ###################################################################
         )
    #elif "FlowRho" in geo_head_type:
    #    flow_out_dim, rho_out_dim, mask_out_dim = get_xyz_mask_flow_rho_out_dim(cfg)
    #    region_num_classes = net_cfg.NUM_CLASSES if geo_head_cfg.REGION_CLASS_AWARE else 1
    #    flow_num_classes = net_cfg.NUM_CLASSES if geo_head_cfg.FLOW_CLASS_AWARE else 1
    #    rho_num_classes = net_cfg.NUM_CLASSES if geo_head_cfg.RHO_CLASS_AWARE else 1
    #    geo_head_init_cfg.update(
    ...
    if geo_head_cfg.FREEZE:
        # print("********** geo_head_cfg.FREEZEgeo_head_cfg.FREEZEgeo_head_cfg.FREEZEpgeo_head_cfg.FREEZEgeo_head_cfg.FREEZE *********:",geo_head_cfg.FREEZE)
...
def get_pnp_net(cfg):
    ...
    if rho_out_dim:
        pnp_net_in_channel += 3    ######### SHOULD BE +=3 if if flow_out_dim : pnp_net_in_channel = 3 is commented
    ...
    # if pnp_net_cfg.REGION_ATTENTION:
    #     # pnp_net_in_channel = g_head_cfg.NUM_REGIONS
    #     pnp_net_in_channel = g_head_cfg.NUM_REGIONS      ######### SHOUDL BE += g_head_cfg.NUM_REGIONS if using both flow and/or rho. OR = if flow and rho are commented
    ...
    if pnp_net_cfg.FREEZE:
        # print("********** pnp_net_cfg.FREEZEpnp_net_cfg.FREEZEpnp_net_cfg.FREEZEpnp_net_cfg.FREEZEpnp_net_cfg.FREEZE *********:",pnp_net.parameters())
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Model Utils !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        # import sys
        # sys.exit()
        for param in pnp_net.parameters():
            with torch.no_grad():
                param.requires_grad = False
                # print("param.requires_gradparam.requires_gradparam.requires_gradparam.requires_gradparam.requires_gradparam.requires_grad:  ", param.requires_grad)
    else:
        params_lr_list.append(
            {
