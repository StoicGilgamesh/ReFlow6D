...
class TopDownMaskFlowRhoRegionHead(nn.Module):
    def __init__(
        self,
        in_dim,
        ...
        if self.out_layer_shared:
            ...
            ##################################################### ABALATION STUDIES ######################################################
            self.flow_out_layer = nn.Conv2d(
                feat_dim,
                self.flow_out_dim * self.flow_num_classes,
                kernel_size=out_kernel_size,
                padding=(out_kernel_size - 1) // 2,
                bias=True,
            )
            self.rho_out_layer = nn.Conv2d(
                feat_dim,
                self.rho_out_dim * self.rho_num_classes,
                kernel_size=out_kernel_size,
                padding=(out_kernel_size - 1) // 2,
                bias=True,
            )
        ...
        if self.out_layer_shared:
            ...
            ##################################################### ABALATION STUDIES ######################################################
            normal_init(self.flow_out_layer, std=0.01)
            normal_init(self.rho_out_layer, std=0.01)
            ##################################################### ABALATION STUDIES ######################################################
    ...
    def forward(self, x):
        ...
        if self.out_layer_shared:
            ...
            ##################################################### ABALATION STUDIES ######################################################
            flow = self.flow_out_layer(x)
            rho = self.rho_out_layer(x)
            ##################################################### ABALATION STUDIES ######################################################

        # return mask, coor_x, coor_y, coor_z, region
        ##################################################### ABALATION STUDIES ######################################################
        return mask, region, flow, rho
        # return mask, region
        ##################################################### ABALATION STUDIES ######################################################
