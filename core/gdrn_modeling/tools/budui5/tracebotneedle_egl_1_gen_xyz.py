...
idx2class = {i: str(i) for i in range(3, 5)}
...
data_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/tracebotneedle/train_pbr"))

cls_indexes = [_idx for _idx in sorted(idx2class.keys())]
cls_names = [idx2class[cls_idx] for cls_idx in cls_indexes]
model_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/tracebotneedle/models"))
...
scenes = [f"{i:06d}" for i in range(3,72)]
#scenes = ['000001']
                ...
                for anno_i, anno in enumerate(annos):
                    ...
                    if mask.sum() == 0:  # NOTE: this should be ignored at training phase
                        ...
                        if VIS:
                            ...
                            np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/bottlearch/bgr_gl.npy",bgr_gl[:, :, [2, 1, 0]])
                            np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/bottlearch/im.npy", im[:, :, [2, 1, 0]])
                            np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/bottlearch/xyz_th_cpu.npy", xyz_th_cpu)
                            np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/bottlearch/xyz_crop.npy", xyz_crop)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="gen tracebotneedle pbr xyz")
