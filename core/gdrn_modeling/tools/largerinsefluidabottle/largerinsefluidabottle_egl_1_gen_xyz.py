...
near = 0.25
far = 6.0
data_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/largerinsefluidabottle/train_pbr"))

cls_indexes = [_idx for _idx in sorted(idx2class.keys())]
cls_names = [idx2class[cls_idx] for cls_idx in cls_indexes]
model_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/largerinsefluidabottle/models"))
        ...
        for scene in tqdm(scenes):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                for anno_i, anno in enumerate(annos):
                    ...
                    #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_xyz/image_tensor.npy", self.image_tensor.cpu().numpy())
                    #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_xyz/seg_tensor.npy", self.seg_tensor.cpu().numpy())
                    #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_xyz/pc_cam_tensor.npy", self.pc_cam_tensor.cpu().numpy())
                    ...
                    if mask.sum() == 0:  # NOTE: this should be ignored at training phase
                        ...
                        if VIS:
                            ...
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/largerinsefluidabottle/bgr_gl.npy",bgr_gl[:, :, [2, 1, 0]])
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/largerinsefluidabottle/im.npy", im[:, :, [2, 1, 0]])
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/largerinsefluidabottle/mask_gt.npy", mask_gt)
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/largerinsefluidabottle/mask_visib_gt.npy", mask_visib_gt)
                            show_titles = [
                                "bgr_gl",
                                "im",
                                "mask_gt",
                        ...
                        if VIS:
                            ...
                            grid_show(show_ims, show_titles, row=2, col=2)
                        #print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! largerinsefluidabottle gen xyz !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                        #import sys
                        #sys.exit()
