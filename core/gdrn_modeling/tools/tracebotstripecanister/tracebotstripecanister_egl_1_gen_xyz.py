...
near = 0.25
far = 6.0
data_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/tracebotstripecanister/train_pbr"))

cls_indexes = [_idx for _idx in sorted(idx2class.keys())]
cls_names = [idx2class[cls_idx] for cls_idx in cls_indexes]
model_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/tracebotstripecanister/models"))
...
scenes = [f"{i:06d}" for i in range(7)]
    ...
    def main(self):
        for scene in tqdm(scenes):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                #im_path = osp.join(data_dir, scene, f"rgb/{int_im_id:06d}.jpg")
                ...
                for anno_i, anno in enumerate(annos):
                    ...
                    # np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_xyz/image_tensor.npy", self.image_tensor.cpu().numpy())
                    # np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_xyz/seg_tensor.npy", self.seg_tensor.cpu().numpy())
                    # np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_xyz/pc_cam_tensor.npy", self.pc_cam_tensor.cpu().numpy())
                    # np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_xyz/K.npy", K)
                    # np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_xyz/pose.npy", pose)
                    # np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_xyz/render_obj_id.npy", render_obj_id)
                    # np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_xyz/im_path.npy", im_path)
                    
                    
                    # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! fluidcontainer gen xyz !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    # # import sys
                    # sys.exit()
                    ...
                    if mask.sum() == 0:  # NOTE: this should be ignored at training phase
                        ...
                        if VIS:
                            ...
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/fluidcontainer/bgr_gl.npy",bgr_gl[:, :, [2, 1, 0]])
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/fluidcontainer/im.npy", im[:, :, [2, 1, 0]])
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/fluidcontainer/mask_gt.npy", mask_gt)
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/core/gdrn_modeling/tools/fluidcontainer/mask_visib_gt.npy", mask_visib_gt)
                            show_titles = [
                                "bgr_gl",
                                "im",
                                "mask_gt",
                        ...
                        if VIS:
                            ...
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_xyz/bgr_gl.npy",bgr_gl[:, :, [2, 1, 0]])
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_xyz/im.npy", im[:, :, [2, 1, 0]])
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_xyz/xyz_th_cpu.npy", xyz_th_cpu)
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_xyz/xyz_crop.npy", xyz_crop)
                            #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_xyz/xyxy.npy", [x1, y1, x2, y2])
