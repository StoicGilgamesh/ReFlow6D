        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.mmmmm2.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_MMMMM2 = dict(
    mmmmm2_bop_test=dict(
        name="mmmmm2_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mmmmm2/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mmmmm2/models"),
        objs=ref.mmmmm2.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/mmmmm2/test_targets_bop19.json"),
...
for obj in ref.mmmmm2.objects:
    name = "mmmmm2_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_MMMMM2:
        SPLITS_MMMMM2[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mmmmm2/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mmmmm2/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_MMMMM2:
        used_cfg = SPLITS_MMMMM2[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, MMMMM2_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="mmmmm2",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_MMMMM2.keys())
