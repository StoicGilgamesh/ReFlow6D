        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.heart0.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_HEART0 = dict(
    heart0_bop_test=dict(
        name="heart0_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/heart0/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/heart0/models"),
        objs=ref.heart0.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/heart0/test_targets_bop19.json"),
...
for obj in ref.heart0.objects:
    name = "heart0_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_HEART0:
        SPLITS_HEART0[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/heart0/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/heart0/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_HEART0:
        used_cfg = SPLITS_HEART0[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, HEART0_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="heart0",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_HEART0.keys())
