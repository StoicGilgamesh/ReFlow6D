...
class BOTTLE1_BOP_TEST_Dataset:
    """bottle1 bop test."""
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.bottle1.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_BOTTLE1 = dict(
    bottle1_bop_test=dict(
        name="bottle1_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bottle1/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bottle1/models"),
        objs=ref.bottle1.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/bottle1/test_targets_bop19.json"),
...
for obj in ref.bottle1.objects:
    name = "bottle1_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_BOTTLE1:
        SPLITS_BOTTLE1[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bottle1/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bottle1/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_BOTTLE1:
        used_cfg = SPLITS_BOTTLE1[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, BOTTLE1_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="bottle1",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_BOTTLE1.keys())
