...
class BUDUI1_BOP_TEST_Dataset:
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/budui1/test
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.budui1.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_BUDUI1 = dict(
    budui1_bop_test=dict(
        name="budui1_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui1/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui1/models"),
        objs=ref.budui1.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui1/test_targets_bop19.json"),
...
for obj in ref.budui1.objects:
    name = "budui1_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_BUDUI1:
        SPLITS_BUDUI1[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui1/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui1/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_BUDUI1:
        used_cfg = SPLITS_BUDUI1[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, BUDUI1_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="budui1",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_BUDUI1.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m core.datasets.budui1_bop_test dataset_name
    """
