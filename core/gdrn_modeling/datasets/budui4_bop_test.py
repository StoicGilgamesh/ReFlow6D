        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.budui4.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_BUDUI4 = dict(
    budui4_bop_test=dict(
        name="budui4_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui4/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui4/models"),
        objs=ref.budui4.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui4/test_targets_bop19.json"),
...
for obj in ref.budui4.objects:
    name = "budui4_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_BUDUI4:
        SPLITS_BUDUI4[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui4/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui4/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_BUDUI4:
        used_cfg = SPLITS_BUDUI4[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, BUDUI4_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="budui4",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_BUDUI4.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m core.datasets.budui4_bop_test dataset_name
    """
