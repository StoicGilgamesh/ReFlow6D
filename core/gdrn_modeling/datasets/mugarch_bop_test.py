...
class MUGARCH_BOP_TEST_Dataset:
    """bottle bop test."""
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.mugarch.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_MUGARCH = dict(
    mugarch_bop_test=dict(
        name="mugarch_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mugarch/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mugarch/models"),
        objs=ref.mugarch.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/mugarch/test_targets_bop19.json"),
...
for obj in ref.mugarch.objects:
    name = "mugarch_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_MUGARCH:
        SPLITS_MUGARCH[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mugarch/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mugarch/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_MUGARCH:
        used_cfg = SPLITS_MUGARCH[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, MUGARCH_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="mugarch",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_MUGARCH.keys())
