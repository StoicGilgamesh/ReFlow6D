...
class BOTTLEARCH_BOP_TEST_Dataset:
    """bottlearch bop test."""
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.bottlearch.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_BOTTLEARCH = dict(
    bottlearch_bop_test=dict(
        name="bottlearch_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bottlearch/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bottlearch/models"),
        objs=ref.bottlearch.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/bottlearch/test_targets_bop19.json"),
...
for obj in ref.bottlearch.objects:
    name = "bottlearch_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_BOTTLEARCH:
        SPLITS_BOTTLEARCH[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bottlearch/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bottlearch/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_BOTTLEARCH:
        used_cfg = SPLITS_BOTTLEARCH[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, BOTTLEARCH_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="bottlearch",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_BOTTLEARCH.keys())
