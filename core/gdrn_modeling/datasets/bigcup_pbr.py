...
class BIGCUP_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get(
            "dataset_root",
            osp.join(DATASETS_ROOT, "BOP_DATASETS/bigcup/train_pbr"),
        )
        ...
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/bigcup/models
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.bigcup.id2obj.items() if obj_name in self.objs]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.bigcup.obj2id[obj_name]:06d}.ply",
                ),
...
def get_bigcup_metadata(obj_names, ref_key):
...
bigcup_model_root = "BOP_DATASETS/bigcup/models/"
################################################################################


SPLITS_BIGCUP_PBR = dict(
    bigcup_train_pbr=dict(
        name="bigcup_train_pbr",
        objs=ref.bigcup.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bigcup/train_pbr"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bigcup/models"),
        xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bigcup/train_pbr/xyz_crop"),
...
# single obj splits
for obj in ref.bigcup.objects:
    for split in ["train_pbr"]:
        name = "bigcup_{}_{}".format(obj, split)
        ...
        if name not in SPLITS_BIGCUP_PBR:
            SPLITS_BIGCUP_PBR[name] = dict(
                name=name,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_BIGCUP_PBR:
        used_cfg = SPLITS_BIGCUP_PBR[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, BIGCUP_PBR_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="bigcup_pbr",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_bigcup_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_BIGCUP_PBR.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m this_module bigcup_train_pbr
    """
