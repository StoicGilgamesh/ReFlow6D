...
class FLUIDCONTAINER_BOP_TEST_Dataset:
    """fluidcontainer bop test."""
    ...
    def __init__(self, data_cfg):
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.fluidcontainer.id2obj.items() if obj_name in self.objs]
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.fluidcontainer.id2obj[obj_id] not in self.select_objs:
                        continue
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.fluidcontainer.obj2id[obj_name]:06d}.ply",
                ),
...
def get_fluidcontainer_metadata(obj_names, ref_key):
...
SPLITS_FLUIDCONTAINER = dict(
    fluidcontainer_bop_test=dict(
        name="fluidcontainer_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/fluidcontainer/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/fluidcontainer/models"),
        objs=ref.fluidcontainer.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/fluidcontainer/test_targets_bop19.json"),
        ...
        ref_key="fluidcontainer",
    )
)


# single objs (num_class is from all objs)
for obj in ref.fluidcontainer.objects:
    name = "fluidcontainer_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_FLUIDCONTAINER:
        SPLITS_FLUIDCONTAINER[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/fluidcontainer/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/fluidcontainer/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_FLUIDCONTAINER:
        used_cfg = SPLITS_FLUIDCONTAINER[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, FLUIDCONTAINER_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="fluidcontainer",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_fluidcontainer_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_FLUIDCONTAINER.keys())
