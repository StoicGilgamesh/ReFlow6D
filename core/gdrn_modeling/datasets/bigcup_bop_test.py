...
class BIGCUP_BOP_TEST_Dataset:
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/bigcup/test
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.bigcup.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_BIGCUP = dict(
    bigcup_bop_test=dict(
        name="bigcup_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bigcup/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bigcup/models"),
        objs=ref.bigcup.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/bigcup/test_targets_bop19.json"),
...
for obj in ref.bigcup.objects:
    name = "bigcup_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_BIGCUP:
        SPLITS_BIGCUP[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bigcup/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/bigcup/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_BIGCUP:
        used_cfg = SPLITS_BIGCUP[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, BIGCUP_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="bigcup",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_BIGCUP.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m core.datasets.bigcup_bop_test dataset_name
    """
