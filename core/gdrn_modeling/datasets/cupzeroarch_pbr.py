...
class CUPZEROARCH_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get(
            "dataset_root",
            osp.join(DATASETS_ROOT, "BOP_DATASETS/cupzeroarch/train_pbr"),
        )
...
cupzeroarch_model_root = "BOP_DATASETS/cupzeroarch/models/"
################################################################################


SPLITS_CUPZEROARCH_PBR = dict(
    cupzeroarch_train_pbr=dict(
        name="cupzeroarch_train_pbr",
        objs=ref.cupzeroarch.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/cupzeroarch/train_pbr"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/cupzeroarch/models"),
        xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/cupzeroarch/train_pbr/xyz_crop"),
...
for obj in ref.cupzeroarch.objects:
    for split in ["train_pbr"]:
        name = "cupzeroarch_{}_{}".format(obj, split)
        ...
        if name not in SPLITS_CUPZEROARCH_PBR:
            SPLITS_CUPZEROARCH_PBR[name] = dict(
                name=name,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_CUPZEROARCH_PBR:
        used_cfg = SPLITS_CUPZEROARCH_PBR[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, CUPZEROARCH_PBR_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="cupzeroarch_pbr",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_CUPZEROARCH_PBR.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m this_module cupzeroarch_train_pbr
    """
