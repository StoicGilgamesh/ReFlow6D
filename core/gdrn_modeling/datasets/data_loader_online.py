...
class GDRN_Online_DatasetFromList(Base_DatasetFromList):
    ...
    def __init__(self, cfg, split, lst: list, copy: bool = True, serialize: bool = True, flatten=True):
        ...
        ############################################### FOR MATTING LOSS ##############################################################
        self.matting_coco_list = glob.glob('/PhD_Stuff/DATASETS/coco2017/test2017/*.jpg') ### PATH TO COCO IMAGES  /hrishi/PhD_Stuff/DATASETS/coco2017/test2017
        ############################################### FOR MATTING LOSS ##############################################################
...
#                    image = self._color_aug(image, self.color_aug_type)
        ...
        # rho_image = read_image_mmcv(anno["rho_path"], format=self.img_format)
        # flow_image = read_image_mmcv(anno["flow_path"], format=self.img_format)
        ...
        roi_img_debug = crop_resize_by_warp_affine(
            image, bbox_center, scale, input_res, interpolation=cv2.INTER_LINEAR
        ).transpose(2, 0, 1)
        ...
        # INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
        ## flow_img ------------------------------------
        dataset_dict["file_path"] = dataset_dict["file_name"]
        # dataset_dict["anno_i"] = anno["anno_i"]
        # np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_input/flow_image.npy",flow_image)
        # np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_input/image.npy",image)
        # print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> FILENAME  >>>>>>>>>>>>>>>>>>>>>>>: ",dataset_dict["file_name"])
        flow_img = crop_resize_by_warp_affine(
            flow_image, bbox_center, scale, out_res, interpolation=cv2.INTER_LINEAR
        ).transpose(2, 0, 1)
        flow_img_post_norm = self.normalize_image(cfg, flow_img)
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! flow_img !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!: ",np.all(flow_img[:,:,0]==1.0))
        # np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_input/flow_img.npy",flow_img)
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! file_path  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!: ",dataset_dict["file_path"] )
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! anno_i !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!: ",dataset_dict["anno_i"])
        # if np.all(flow_img.T[:,:,0]==255) and np.all(flow_img.T[:,:,1]==255) and np.all(flow_img.T[:,:,2]==255):
        #     np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_input/flow_img.npy",flow_img)
        #     np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_input/flow_img_post_norm.npy",flow_img_post_norm)
        #     np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_input/roi_img_debug.npy",roi_img_debug)
        #     np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_input/dataset_dict.npy",dataset_dict)
        #     np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_input/mask_full.npy",anno["mask_full"])
        #     print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! data_loader_online !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  ", dataset_dict)
        #     print("\n !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! data_loader_online annotations !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  ", anno)
        #     print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! file_path  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!: ",dataset_dict["file_path"] )
        #     print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! anno_i !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!: ",dataset_dict["anno_i"])
        #     print("\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! anno flow_path !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!: ",anno["flow_path"])
        #     print("\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! anno mask_visib_file !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!: ",anno["mask_visib_file"])
        ...
        flow_img = self.normalize_image(cfg, flow_img)
        ## rho_img ------------------------------------
        rho_img = crop_resize_by_warp_affine(
            rho_image, bbox_center, scale, out_res, interpolation=cv2.INTER_LINEAR
        ).transpose(2, 0, 1)

        rho_img = self.normalize_image(cfg, rho_img)
        
        
        ############################################### FOR MATTING LOSS ##############################################################
        
        # print("SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS: ", self.matting_coco_list)
        selected_matt_img_coco = random.sample(self.matting_coco_list, 1)
        matt_img_coco = read_image_mmcv(selected_matt_img_coco[0], format=self.img_format)
        matt_img_coco = cv2.resize(matt_img_coco, (100, 100), interpolation = cv2.INTER_AREA)
        matt_img_coco = matt_img_coco.astype(np.uint8)


        # matt_img_coco = self.normalize_image(cfg, matt_img_coco.T) # MAYBE NO NORMALIZATION
        ############################################### FOR MATTING LOSS ##############################################################
        
        # INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
        
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/rho_image_dataloader.npy",rho_image)
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/flow_image_dataloader.npy",flow_image)
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/image_dataloader.npy",image)
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/rho_img_dataloader.npy",rho_img)
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/flow_img_dataloader.npy",flow_img)
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/roi_img_dataloader.npy",roi_img)
        ...
        if g_head_cfg.NUM_REGIONS > 1:
            ...
            roi_region = roi_region.transpose(1, 0)  # HW
        ...
        dataset_dict["matting_coco_img"] = matt_img_coco.astype("uint8")
        ...
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/roi_mask_obj_dataloader.npy",roi_mask_obj.astype("float32"))
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/roi_mask_full_dataloader.npy", roi_mask_full.astype("float32"))
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/roi_mask_trunc_dataloader.npy", roi_mask_trunc.astype("float32"))
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/roi_mask_visib_dataloader.npy", roi_mask_visib.astype("float32"))
        np.save("/PhD_Stuff/GDRN_TRANSPARENT/gdrn/debug_output/roi_xyz_dataloader.npy", roi_xyz.astype("float32"))
