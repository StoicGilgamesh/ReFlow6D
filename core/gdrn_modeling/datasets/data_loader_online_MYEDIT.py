...
# INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
    def CALC_BBOX_AGAIN(mask_img):
        contours = measure.find_contours(mask_img, 0.5, positive_orientation='low')
...
# INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
    ...
    def read_data_train(self, dataset_dict):
        ...
        dataset_dict["image_file_name"] = dataset_dict["file_name"]    
        if self.img_format == "L":
            image = np.expand_dims(image, 2).repeat(3, axis=2)
            
            
        rho_image = read_image_mmcv(dataset_dict["rho_file"], format=self.img_format)
        flow_image = read_image_mmcv(dataset_dict["flow_file"], format=self.img_format)
        ...
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/image_natural_before_1.npy", image)
...
#                    image = self._color_aug(image, self.color_aug_type)
        ...
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/image_natural_before_2.npy", image)
        ...
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/image_natural_before_3.npy", image)
        ...
        # INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
        contours = measure.find_contours( (anno["mask_full"].astype("float32")).T, 0.5, positive_orientation='low')
        ...
        width = max_x - x
        height = max_y - y
        bbox_cald_agn = (x, y, width, height)


        # CHW, float32 tensor
        # ## roi_image ------------------------------------
        # roi_img_old = crop_resize_by_warp_affine(
        #     image, bbox_center, scale, input_res, interpolation=cv2.INTER_LINEAR
        # ).transpose(2, 0, 1)

        # roi_img_old = self.normalize_image(cfg, roi_img_old)

        #roi_img = crop_resize_by_warp_affine(
        #    image, bbox_center, scale, input_res, interpolation=cv2.INTER_LINEAR
        #)

        #roi_img = self.normalize_image(cfg, roi_img.T)
        ...
        square_x = max(center_x - half_size, 0)
        square_y = max(center_y - half_size, 0)

        # Calculate the bottom-right corner of the square
        square_x2 = min(center_x + half_size, image.shape[1])
        square_y2 = min(center_y + half_size, image.shape[0])

        # Calculate the actual size of the square
        square_size = min(square_x2 - square_x, square_y2 - square_y)

        # Adjust the square size if necessary to ensure the cropped image is 256x256
        if square_size < 256:
            square_size = 256
            half_size = 128

        # Check if the square is too close to the left or top edge of the image
        if square_x == 0:
        ...
        # Check if the square is too close to the right or bottom edge of the image
        if square_x2 == image.shape[1]:
            square_x = image.shape[1] - square_size
            square_x2 = image.shape[1]
        if square_y2 == image.shape[0]:
            square_y = image.shape[0] - square_size
            square_y2 = image.shape[0]


        # Crop the image
        roi_img = image[int(square_y):int(square_y)+int(square_size), int(square_x):int(square_x)+int(square_size)]



        # roi_img = image[int(y):int(y)+int(height), int(x):int(x)+int(width)]#a = image[Y:Y+H, X:X+W]
        # roi_img = cv2.resize(roi_img, (input_res, input_res), interpolation = cv2.INTER_AREA)
        roi_img = self.normalize_image(cfg, roi_img.T)
        
        np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/roi_img.npy",roi_img)
        
        # INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
        ## flow_img ------------------------------------
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/flow_img_natural.npy", flow_image)
        flow_img = cv2.resize(flow_image, (out_res, out_res), interpolation = cv2.INTER_AREA)
        #flow_img = flow_img.transpose(2, 0, 1)
        flow_img = flow_img
        #flow_img = crop_resize_by_warp_affine(
        #    flow_image, bbox_center, scale, out_res, interpolation=cv2.INTER_LINEAR
        #).transpose(2, 0, 1)

        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/flow_img_dataonline.npy", flow_img)
        flow_img = self.normalize_image(cfg, flow_img.T)
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/flow_img_dataonline_normalize.npy", flow_img)
        ## rho_img ------------------------------------
        rho_img = cv2.resize(rho_image, (out_res, out_res), interpolation = cv2.INTER_AREA)
        #rho_img = rho_img.transpose(2, 0, 1)
        rho_img = rho_img
        #rho_img = crop_resize_by_warp_affine(
        #    rho_image, bbox_center, scale, out_res, interpolation=cv2.INTER_LINEAR
        #).transpose(2, 0, 1)

        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/rho_img_dataonline.npy", rho_img)
        rho_img = self.normalize_image(cfg, rho_img.T)
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/rho_img_dataonline_normalize.npy", rho_img)
        # INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
        
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_/rho_image_dataloader.npy",rho_image)
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_/flow_image_dataloader.npy",flow_image)
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_/image_dataloader.npy",image)
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_/rho_img_dataloader.npy",rho_img)
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_/flow_img_dataloader.npy",flow_img)
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_/roi_img_dataloader.npy",roi_img)
        ...
        if self.with_depth:
            ...
            #)
            roi_depth = depth[int(y):int(y)+int(height), int(x):int(x)+int(width)]#a = image[Y:Y+H, X:X+W]
            roi_depth = cv2.resize(roi_depth, (input_res, input_res), interpolation = cv2.INTER_AREA)
            if depth_ch == 1:
       ...
       # ).transpose(2, 0, 1)
        roi_coord_2d = coord_2d[int(y):int(y)+int(height), int(x):int(x)+int(width)]#a = image[Y:Y+H, X:X+W]
        roi_coord_2d = cv2.resize(roi_coord_2d, (out_res, out_res), interpolation = cv2.INTER_AREA)
        ...
        #)
        roi_mask_trunc = mask_trunc[:, :, None][int(y):int(y)+int(height), int(x):int(x)+int(width)]#a = image[Y:Y+H, X:X+W]
        roi_mask_trunc = cv2.resize(roi_mask_trunc, (out_res, out_res), interpolation = cv2.INTER_AREA)
        #roi_mask_trunc = roi_mask_trunc.transpose(1,0)
        ...
        #)
        roi_mask_visib = mask_visib[:, :, None][int(y):int(y)+int(height), int(x):int(x)+int(width)]#a = image[Y:Y+H, X:X+W]
        roi_mask_visib = cv2.resize(roi_mask_visib, (out_res, out_res), interpolation = cv2.INTER_AREA)
        #roi_mask_visib = roi_mask_visib.transpose(1,0)
        
        roi_mask_obj = mask_obj[:, :, None][int(y):int(y)+int(height), int(x):int(x)+int(width)]#a = image[Y:Y+H, X:X+W]
        roi_mask_obj = cv2.resize(roi_mask_obj, (out_res, out_res), interpolation = cv2.INTER_AREA)
        ...
        if "mask_full" in anno.keys():
            ...
            roi_mask_full = mask_full[int(y):int(y)+int(height), int(x):int(x)+int(width)]#a = image[Y:Y+H, X:X+W]
            roi_mask_full = cv2.resize(roi_mask_full, (out_res, out_res), interpolation = cv2.INTER_AREA)
            #roi_mask_full = roi_mask_full.transpose(1,0)

        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_/mask_full_dataloader.npy",mask_full)
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_/roi_mask_full_dataloader.npy", roi_mask_full)
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_/roi_mask_trunc_dataloader.npy", roi_mask_trunc)
        #np.save("/RoadmapWithStefan/gdrnpp_bop2022/debug_/roi_mask_visib_dataloader.npy", roi_mask_visib)
        ...
        roi_xyz = xyz[int(y):int(y)+int(height), int(x):int(x)+int(width)]#a = image[Y:Y+H, X:X+W]
        roi_xyz = cv2.resize(roi_xyz, (out_res, out_res), interpolation = cv2.INTER_AREA)
        # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/roi_xyz_dataloader.npy",roi_xyz)
        ...
        if g_head_cfg.NUM_REGIONS > 1:
            ...
            # np.save("/PhD/gdrnpp_bop2022_experimentation_2/bottlearch_exp2_debug/roi_region.npy",roi_region)
        ...
        # INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
        dataset_dict["flow_img"] = torch.as_tensor(flow_img.astype("float32")).contiguous()
        dataset_dict["rho_img"] = torch.as_tensor(rho_img.astype("float32")).contiguous()
        # INTERMEDIATE REPRESENTATION REFLECTIVE ############################################
        ...
        dataset_dict["roi_mask_trunc"] = torch.as_tensor((roi_mask_trunc.astype("float32")).T).contiguous()
        dataset_dict["roi_mask_visib"] = torch.as_tensor((roi_mask_visib.astype("float32")).T).contiguous()
        dataset_dict["roi_mask_obj"] = torch.as_tensor((roi_mask_obj.astype("float32")).T).contiguous()
        if "mask_full" in anno.keys():
            dataset_dict["roi_mask_full"] = torch.as_tensor((roi_mask_full.astype("float32")).T).contiguous()
