...
class MMMMM2_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get(
            "dataset_root",
            osp.join(DATASETS_ROOT, "BOP_DATASETS/mmmmm2/train_pbr"),
        )
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.mmmmm2.id2obj.items() if obj_name in self.objs]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.mmmmm2.obj2id[obj_name]:06d}.ply",
                ),
...
def get_mmmmm2_metadata(obj_names, ref_key):
...
MMMMM2_model_root = "BOP_DATASETS/mmmmm2/models/"
################################################################################


SPLITS_MMMMM2_PBR = dict(
    mmmmm2_train_pbr=dict(
        name="mmmmm2_train_pbr",
        objs=ref.mmmmm2.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mmmmm2/train_pbr"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mmmmm2/models"),
        xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mmmmm2/train_pbr/xyz_crop"),
...
# single obj splits
for obj in ref.mmmmm2.objects:
    for split in ["train_pbr"]:
        name = "mmmmm2_{}_{}".format(obj, split)
        ...
        if name not in SPLITS_MMMMM2_PBR:
            SPLITS_MMMMM2_PBR[name] = dict(
                name=name,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_MMMMM2_PBR:
        used_cfg = SPLITS_MMMMM2_PBR[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, MMMMM2_PBR_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="mmmmm2_pbr",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_mmmmm2_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_MMMMM2_PBR.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m this_module mmmmm2_train_pbr
    """
