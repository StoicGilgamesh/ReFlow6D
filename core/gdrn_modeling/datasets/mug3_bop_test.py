        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.mug3.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_MUG3 = dict(
    mug3_bop_test=dict(
        name="mug3_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mug3/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mug3/models"),
        objs=ref.mug3.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/mug3/test_targets_bop19.json"),
...
for obj in ref.mug3.objects:
    name = "mug3_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_MUG3:
        SPLITS_MUG3[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mug3/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mug3/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_MUG3:
        used_cfg = SPLITS_MUG3[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, MUG3_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="mug3",  # NOTE: for pvnet to determine module
