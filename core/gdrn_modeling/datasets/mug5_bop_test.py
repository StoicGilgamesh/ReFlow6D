        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.mug5.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_MUG5 = dict(
    mug5_bop_test=dict(
        name="mug5_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mug5/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mug5/models"),
        objs=ref.mug5.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/mug5/test_targets_bop19.json"),
...
for obj in ref.mug5.objects:
    name = "mug5_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_MUG5:
        SPLITS_MUG5[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mug5/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mug5/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_MUG5:
        used_cfg = SPLITS_MUG5[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, MUG5_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="mug5",  # NOTE: for pvnet to determine module
