...
class TRACEBOTCANISTER_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get(
            "dataset_root",
            osp.join(DATASETS_ROOT, "BOP_DATASETS/tracebotcanister/train_pbr"),
        )
        ...
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/bottle/models
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.tracebotcanister.id2obj.items() if obj_name in self.objs]
        ...
        self.scenes = [f"{i:06d}" for i in range(48)]
    ...
    def __call__(self):
        ...
        for scene in tqdm(self.scenes):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                # rho_path = osp.join(scene_root, "rho/{:06d}.png".format(int_im_id))
                # flow_path = osp.join(scene_root, "flow/{:06d}.png".format(int_im_id))
                # rho_path = osp.join(scene_root, "rho/{:06d}_{:06d}.png".format(int_im_id, anno_i)
                # flow_path = osp.join(scene_root, "flow/{:06d}_{:06d}.png".format(int_im_id, anno_i)
                ...
                record = {
                    ...
                    # "rho_file": osp.relpath(rho_path, PROJ_ROOT),
                    # "flow_file": osp.relpath(flow_path, PROJ_ROOT),
                    "height": self.height,
                    "width": self.width,
                    "image_id": int_im_id,
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    record["rho_file"] = osp.relpath(rho_path, PROJ_ROOT)
                    record["flow_file"] = osp.relpath(flow_path, PROJ_ROOT)
                    ...
                    inst = {
                        ...
                        "rho_path":osp.relpath(rho_path, PROJ_ROOT),
                        "flow_path":osp.relpath(flow_path, PROJ_ROOT),
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.tracebotcanister.obj2id[obj_name]:06d}.ply",
                ),
...
def get_tracebotcanister_metadata(obj_names, ref_key):
...
tracebotcanister_model_root = "BOP_DATASETS/tracebotcanister/models/"
################################################################################


SPLITS_TRACEBOTCANISTER_PBR = dict(
    tracebotcanister_train_pbr=dict(
        name="tracebotcanister_train_pbr",
        objs=ref.tracebotcanister.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/tracebotcanister/train_pbr"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/tracebotcanister/models"),
        xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/tracebotcanister/train_pbr/xyz_crop"),
        ...
        num_to_load=-1,
        filter_invalid=True,
        ref_key="tracebotcanister",
    )
)

# single obj splits
for obj in ref.tracebotcanister.objects:
    for split in ["train_pbr"]:
        name = "tracebotcanister_{}_{}".format(obj, split)
        ...
        if name not in SPLITS_TRACEBOTCANISTER_PBR:
            SPLITS_TRACEBOTCANISTER_PBR[name] = dict(
                name=name,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_TRACEBOTCANISTER_PBR:
        used_cfg = SPLITS_TRACEBOTCANISTER_PBR[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, TRACEBOTCANISTER_PBR_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="tracebotcanister_pbr",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_tracebotcanister_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_TRACEBOTCANISTER_PBR.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m this_module tracebotcanister_train_pbr
    """
