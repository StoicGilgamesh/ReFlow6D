...
class LARGERINSEFLUIDABOTTLE_BOP_TEST_Dataset:
    """largerinsefluidabottle bop test."""
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/bottle/test
    ...
    def __call__(self):
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                rho_path = osp.join(scene_root, "rho/{:06d}.png".format(im_id))
                
                flow_path = osp.join(scene_root, "flow/{:06d}.png".format(im_id))
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.largerinsefluidabottle.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_LARGERINSEFLUIDABOTTLE = dict(
    largerinsefluidabottle_bop_test=dict(
        name="largerinsefluidabottle_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/largerinsefluidabottle/test"),
        ...
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/largerinsefluidabottle/test_targets_bop19.json"),
...
for obj in ref.largerinsefluidabottle.objects:
    name = "largerinsefluidabottle_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_LARGERINSEFLUIDABOTTLE:
        SPLITS_LARGERINSEFLUIDABOTTLE[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/largerinsefluidabottle/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/largerinsefluidabottle/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_LARGERINSEFLUIDABOTTLE:
        used_cfg = SPLITS_LARGERINSEFLUIDABOTTLE[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, LARGERINSEFLUIDABOTTLE_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="largerinsefluidabottle",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_LARGERINSEFLUIDABOTTLE.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m core.datasets.bottle_bop_test dataset_name
    """
