...
class BUDUI5_BOP_TEST_Dataset:
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/budui5/test
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.budui5.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_BUDUI5 = dict(
    budui5_bop_test=dict(
        name="budui5_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui5/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui5/models"),
        objs=ref.budui5.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui5/test_targets_bop19.json"),
...
for obj in ref.budui5.objects:
    name = "budui5_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_BUDUI5:
        SPLITS_BUDUI5[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui5/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/budui5/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_BUDUI5:
        used_cfg = SPLITS_BUDUI5[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, BUDUI5_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="budui5",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_BUDUI5.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m core.datasets.budui5_bop_test dataset_name
    """
