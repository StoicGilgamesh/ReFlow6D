...
class ZHUIXIN_BOP_TEST_Dataset:
    """zhuixin bop test."""
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/zhuixin/test
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/zhuixin/models
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.zhuixin.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_ZHUIXIN = dict(
    zhuixin_bop_test=dict(
        name="zhuixin_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/zhuixin/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/zhuixin/models"),
        objs=ref.zhuixin.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/zhuixin/test_targets_bop19.json"),
...
for obj in ref.zhuixin.objects:
    name = "zhuixin_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_ZHUIXIN:
        SPLITS_ZHUIXIN[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/zhuixin/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/zhuixin/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_ZHUIXIN:
        used_cfg = SPLITS_ZHUIXIN[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, ZHUIXIN_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="zhuixin",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_ZHUIXIN.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m core.datasets.zhuixin_bop_test dataset_name
    """
