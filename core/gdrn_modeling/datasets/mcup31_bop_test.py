...
class MCUP31_BOP_TEST_Dataset:
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/mcup31/test
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno["obj_id"]
                    if ref.mcup31.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_MCUP31 = dict(
    mcup31_bop_test=dict(
        name="mcup31_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mcup31/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mcup31/models"),
        objs=ref.mcup31.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/mcup31/test_targets_bop19.json"),
...
for obj in ref.mcup31.objects:
    name = "mcup31_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_MCUP31:
        SPLITS_MCUP31[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mcup31/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mcup31/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_MCUP31:
        used_cfg = SPLITS_MCUP31[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, MCUP31_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="mcup31",  # NOTE: for pvnet to determine module
...
def get_available_datasets():
    return list(SPLITS_MCUP31.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m core.datasets.mcup31_bop_test dataset_name
    """
