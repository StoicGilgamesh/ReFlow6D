...
OUTPUT_DIR = "output/gdrn/budui1/convnext_a6_AugCosyAAEGray_BG05_mlL1_DMask_amodalClipBox_classAware_budui1"
INPUT = dict(
    DZI_PAD_SCALE=1.5,
    TRUNCATE_FG=True,
    CHANGE_BG_PROB=0.0,
...
DATASETS = dict(
    TRAIN = ["budui1_train_pbr"],
#    TRAIN = ["train"],
    TEST = ["budui1_bop_test"],
    DET_FILES_TEST=("datasets/BOP_DATASETS/budui1/test/test_bboxes/yolox_x_640_budui1.json",),
...
MODEL = dict(
    ...
    POSE_NET=dict(
        ...
        LOSS_CFG=dict(
            ...
            FLOW_LOSS_MASK_GT="obj",  # trunc | visib | obj #### WAS VISIB
            FLOW_LW=1.0,
            # rho loss ----------------------------
            RHO_LOSS_TYPE="L1",  # L1 | CE_coor
            RHO_LOSS_MASK_GT="obj",  # trunc | visib | obj#### WAS VISIB
            RHO_LW=1.0,
            # mask loss ---------------------------
            MASK_LOSS_TYPE="L1",  # L1 | BCE | CE
            MASK_LOSS_GT="obj",  # trunc | visib | gt
            MASK_LW=1.0,
            # full mask loss ---------------------------
            ...
            REGION_LOSS_MASK_GT="obj",  # trunc | visib | obj
            REGION_LW=1.0,
            # pm loss --------------
            PM_LOSS_SYM=False,  # NOTE: sym loss
